...
Please note that a config file generated by exporting an environment with iOS push settings configured, should be modified slightly before usage. Downloading an iOS push certificate previously uploaded to Kinvey is not possible. This is why the generated config file contains only its name (`push.ios.certificateFilename`) but not the certificate itself. To apply the exported config file, `push.ios.certificateFilename` must be replaced with `push.ios.certificateFilePath` (a path to the actual certificate). Another alternative is removing the `push.ios` field but this means that iOS push settings won't be applied or will be removed if the config file is applied to an environment with configured iOS push settings.
