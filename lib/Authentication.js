...
class Authentication {
  ...
  _loginWithExternalIdentityProvider({ identityProvider, host }, done) {
    const endpoint = Endpoints.identityProvidersAuthenticate(this.cliManager.config.defaultSchemaVersion, identityProvider.id);
    const mapiHost = host;
    ...
    const reqHandler = function (req, res) {
      ...
      if (!isNullOrUndefined(parsedQuery.session)) {
  ...
  _login(options, done) {
    ...
    this._loginWithExternalIdentityProvider(options, done);
  }
  ...
  login(options, done) {
    this._login(options, (err, result) => {
      ...
      this.setCurrentUser({
        host: options.host,
        email: result.email, // TODO: kdev-539 get the email when external provider
        token: result.token
      });
