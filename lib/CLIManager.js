  ...
  processCommandResult(result, done) {
    this._clearSession(() => {
      ...
      if (isCallbackProvided) {
        if (!result.isSuccess()) {
          return done(result.getErrorResult());
        } else {
          return done(null, result.getDataResult());
        }
      }

      // no success
      if (!result.isSuccess()) {
        const errResult = result.getErrorResult();
        if (this._outputFormat === OutputFormat.JSON) {
          console.log(JSON.stringify(errResult, null, 2));
        } else {
          this.log(LogLevel.ERROR, `${errResult.error.name}: ${errResult.error.message}`);
        }
      ...
      // success
      if (this._outputFormat === OutputFormat.JSON) {
        console.log(JSON.stringify(result.getDataResult(), null, 2));
        return;
      }

      if (result.humanReadableFormatIsTable()) {
        const tableData = result.getTableData();
        if (Array.isArray(tableData)) {
          this.log(LogLevel.INFO, `Count: ${tableData.length}${os.EOL}`);
        }

        console.table(tableData);
      } else {
        this.log(LogLevel.INFO, result.getUserFriendlyMsg());
      }
    });
  }
