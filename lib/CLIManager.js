...
const { Endpoints, formatHost, getCommandNameFromOptions, getDefaultIdentityProvider, isEmpty, isNullOrUndefined, isValidEmail } = require('./Utils');
...
const sitesRouter = require('./website/sitesRouter');
...
const SitesController = require('./website/SitesController');
...
const SitesService = require('./website/SitesService');
const SiteEnvsService = require('./website/SiteEnvsService');

const AppFileExporter = require('./exporter/AppFileExporter');
const EnvFileExporter = require('./exporter/EnvFileExporterV1');
const OrgFileExporter = require('./exporter/OrgFileExporter');
const ServiceExporter = require('./exporter/ServiceFileExporter');

const AppFileProcessor = require('./AppFileProcessor');
const EnvFileProcessor = require('./EnvFileProcessor');
const OrgFileProcessor = require('./OrgFileProcessor');
...
class CLIManager {
  ...
  _registerControllers() {
    ...
    const sitesService = new SitesService(this);
    const siteEnvsService = new SiteEnvsService(this);

    const envExporter = new EnvFileExporter({
      cliManager: this,
      applicationsService,
      collectionsService,
    ...
    const serviceExporter = new ServiceExporter({ cliManager: this, servicesService });
    const appExporter = new AppFileExporter({ cliManager: this, applicationsService, servicesService, envExporter, serviceExporter });
    const orgExporter = new OrgFileExporter({ cliManager: this, organizationsService, applicationsService, servicesService, appExporter, serviceExporter });
    ...
    this._controllers[Namespace.ORG] = new OrganizationsController({ cliManager: this, organizationsService, exporter: orgExporter });
    this._controllers[Namespace.APP] = new ApplicationsController({ cliManager: this, exporter: appExporter, applicationsService, organizationsService });
    this._controllers[Namespace.ENV] = new EnvironmentsController({ cliManager: this, exporter: envExporter, environmentsService });
    this._controllers[Namespace.SERVICE] = new ServicesController({ cliManager: this, exporter: serviceExporter, applicationsService, organizationsService });
    this._controllers[Namespace.COLL] = new CollectionsController({ cliManager: this, collectionsService, environmentsService });
    this._controllers[Namespace.SITE] = new SitesController({ cliManager: this, sitesService, siteEnvsService, organizationsService, applicationsService });
    ...
    const appFileProcessor = new AppFileProcessor({ cliManager: this, applicationsService, environmentsService, servicesService, envFileProcessor, serviceFileProcessor });
    const orgFileProcessor = new OrgFileProcessor({ cliManager: this, organizationsService, servicesService, applicationsService, serviceFileProcessor, appFileProcessor });

    this._configFileProcessor = new ConfigFileProcessor({ appFileProcessor, envFileProcessor, serviceFileProcessor, orgFileProcessor });
  }
  ...
  _initCommandsManager(args) {
    ...
    this._commandsManager
      ...
      .command(Namespace.SITE, `Manage websites. Run 'kinvey ${Namespace.SITE} -h' for details.`, sitesRouter(this))
      .demand(1, '')
      .strict(true)
      .help('h')
      .alias('h', 'help')
  ...
  executeCommandHandler(ctrl, commandHandler, requirements = [], options) {
    async.series([
      ...
      (next) => {
        const requiresOrg = requirements.includes(CommandRequirement.ORG_AVAILABLE);
        if (!requiresOrg) {
          return setImmediate(next);
        }
        ...
        const activeOrgId = this.getActiveItemId(ActiveItemType.ORG);
        if (isNullOrUndefined(activeOrgId)) {
          return setImmediate(() => { next(new KinveyError(Errors.OrgRequired)); });
        }

        options[OrgOptionsName.ORG] = activeOrgId;
        return setImmediate(next);
      },
      (next) => {
  ...
  processCommandResult(err, result) {
    this._clearSession(() => {
      if (err) {
        ...
        // suggest command before exiting
        const isAuthTokenError = err.name === 'InvalidCredentials' && err.message === 'Authorization token invalid or expired.';
        if (isAuthTokenError) {
          this.log(LogLevel.INFO, `Run 'kinvey ${Namespace.PROFILE} login' to reauthenticate.`);
        }
  ...
  setCurrentUserFromOptions(options, done) {
    ...
    options.host = options[AuthOptionsNames.HOST];
    this.login(options, done);
  }

  /**
   * Issues a login request. If it is a success, sets the current user.
   * @param options
   * @param options.identityProvider
   * @param options.identityProvider.id
   * @param options.identityProvider.name
   * @param [options.email]
   * @param [options.password]
   * @param [options.MFAToken]
   * @param [options.host]
   * @param done
   */
  login(options, done) {
    options.host = options.host || this.config.host;
    const isKinvey = options.identityProvider.name === 'Kinvey';
    if (isKinvey) {
      const emailIsValid = isValidEmail(options.email);
      if (!emailIsValid) {
        return setImmediate(() => { done(new KinveyError(Errors.InvalidEmail)); });
      }
    }

    const debugMsg = isKinvey ? `Logging in user: ${options.email}` : `Logging with ${options.identityProvider.name}`;
    this.log(LogLevel.DEBUG, debugMsg);
    this._authentication.login(options, done);
  }
   ...
   * @returns {*}
   */
  createProfile(profileName, options, done) {
    options.host = options.host || this.config.host;
    ...
    async.series([
      ...
      (next) => {
        ...
        this.sendRequest({ endpoint: Endpoints.user(this.config.defaultSchemaVersion) }, (err, user) => {
          if (err) {
            return next(err);
          }
      ...
      (next) => {
        // save profile
        this._setup.addProfile(profileName, email, token, options.host, options.identityProvider);
        this._setup.save(next);
      }
    ], done);
  }

  reAuthenticateProfile(profileName, password, mfaToken, done) {
    ...
    async.waterfall([
      (next) => {
        const options = {
          identityProvider: profile.identityProvider || getDefaultIdentityProvider(),
          email: profile.email,
          MFAToken: mfaToken,
          host: profile.host,
          password
