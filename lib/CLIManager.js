...
const { formatHost, getCommandNameFromOptions, getDefaultIdentityProvider, isEmpty, isNullOrUndefined, isValidEmail } = require('./Utils');
...
class CLIManager {
  ...
  createProfile(profileName, options, done) {
    ...
    async.series([
      ...
      (next) => {
        // save profile
        this._setup.addProfile(profileName, email, token, options.host);
