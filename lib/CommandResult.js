...
const { isEmpty, isNullOrUndefined, isStringWhitespace } = require('./Utils');
...
class CommandResult {
  ...
  /**
   * Appends an additional message to the user friendly message.
   * @param {String} msg
   * @param {String} [separator]
   */
  setAdditionalMsg(msg, separator = EOL) {
    this.humanReadableFormat.userFriendlyMsg = `${this.humanReadableFormat.userFriendlyMsg}${separator}${msg}`;
    return this;
  }
  ...
  _humanReadableFormatIsTable() {
    const isTable = isStringWhitespace(this.humanReadableFormat.userFriendlyMsg) && (!isNullOrUndefined(this.tableData) || !isNullOrUndefined(this.rawData));
    return isTable;
  }
}
