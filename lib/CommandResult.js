...
const { isEmpty, isNullOrUndefined } = require('./Utils');
...
class CommandResult {
  ...
  _getHumanReadableResult() {
    if (this._humanReadableFormatIsTable()) {
      ...
      const sortData = isList && !isEmpty(tableData) && Object.prototype.hasOwnProperty.call(tableData[0], 'name');
      let unformattedData;
      if (sortData) {
        unformattedData = tableData.sort((x, y) => { return x.name.toLowerCase() > y.name.toLowerCase() ? 1 : 0; });
      } else {
        unformattedData = tableData;
      }

      const formattedTableData = consoleTable.getTable(unformattedData);
      return isList ? `Count: ${tableData.length}${EOL}${EOL}${formattedTableData}` : formattedTableData;
    }
