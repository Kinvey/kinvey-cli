...
const { AppOptionsName, DomainTypes, EntityType, EnvOptionsName, LogLevel, OperationType } = require('./Constants');
...
class ConfigFileProcessor {
  ...
  _create(source, options, done) {
    if (source.configType === 'environment') {
  ...
  process(operationType, options, done) {
    async.waterfall([
      (next) => {
