...
Constants.PromptMessages = {
  ...
  INPUT_IDENTITY_PROVIDER: 'Which identity provider would you like to use?',
...
Constants.Errors = {
  ...
  OrgRequired: {
    NAME: 'OrgRequired',
    MESSAGE: `Organization is required. Please set active org or use the --${Constants.OrgOptionsName.ORG} option.`
  },
  AppOrOrgRequired: {
...
Constants.Mapping = {};
Constants.Mapping[Constants.EntityType.PROFILE] = {
  DETAILS: {
    name: 'name',
    email: 'email',
    token: 'token',
...
Constants.AllowedHooksUserCollection = Object.keys(Constants.CollectionHook);
Constants.AllowedHooksCustomCollection = Constants.AllowedHooksUserCollection.filter(s => s.indexOf('Login') < 0);
