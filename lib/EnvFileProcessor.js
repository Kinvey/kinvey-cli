...
class EnvFileProcessor {
  ...
  _createOrUpdateCollectionHooks(env, sourceHooks, done) {
    ...
    async.each(
      ...
      (collName, next) => {
        ...
        // Modify them sequentially as the backend does not support parallel operations on hooks
        async.eachSeries(
          hookNames,
          (hookName, cb) => {
  ...
  static _groupCollectionsPerOperationType(originalCollections, collections) {
    const groupedEntities = FileProcessorHelper.groupEntitiesPerOperationType(originalCollections, collections);
    ...
    async.series([
      (next) => {
        this.collectionsService.getAll(envId, (err, originalCollections) => {
          ...
          groupedEntities = EnvFileProcessor._groupCollectionsPerOperationType(originalCollections, collections);
          next();
        });
      },
      (next) => {
    ...
    async.series([
      (next) => {
        this.blService.getEndpoints(envId, null, (err, originalEndpoints) => {
          ...
          groupedEntities = FileProcessorHelper.groupEntitiesPerOperationType(originalEndpoints, endpoints);
    ...
    async.series([
      (next) => {
        this.blService.getCommonCode(envId, null, null, (err, originalCommonModules) => {
          ...
          groupedEntities = FileProcessorHelper.groupEntitiesPerOperationType(originalCommonModules, commonCode);
          next();
        });
      },
      (next) => {
  ...
  static _getFilteredCollectionData(sourceCollection) {
    const collBlacklist = ['type', 'service', 'serviceEnvironment', 'serviceObject'];
    ...
    this._findServiceFromSource(sourceColl, targetColl.name, env, (err, data) => {
      ...
      if (data) {
        targetColl.dataLink = {
          ...
          id: data.service.id,
          backingServerId: data.svcEnv.id
        };
      }
  ...
  _findServiceFromSource(source, name, env, done) {
    ...
    const domainId = env.app;
    this._getServiceInfo(source.service, source.serviceEnvironment, domainId, done);
  }
  ...
  _getServiceInfo(serviceIdentifier, svcEnvIdentifier, appId, done) {
    let service;
    let svcEnv;
    let app;
    ...
    async.series([
      (next) => {
        this.applicationsService.getByIdOrName(appId, (err, data) => {
          if (err) {
            return next(err);
          }
      ...
      (next) => {
        this.servicesService.getAllByDomainType(app, null, (err, data) => {
          if (err) {
            return next(err);
          }

          service = data.find(x => x.id === serviceIdentifier || x.name === serviceIdentifier);
          if (!service) {
            return next(getCustomNotFoundError(EntityType.SERVICE, serviceIdentifier));
          }
      ...
      (next) => {
        this.servicesService.getServiceEnvs(service.id, (err, data) => {
          ...
          svcEnv = data.find(x => x.id === svcEnvIdentifier || x.name === svcEnvIdentifier);
          if (!svcEnv) {
            return next(getCustomNotFoundError(EntityType.SCV_ENV, svcEnvIdentifier));
          }
