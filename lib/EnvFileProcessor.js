...
class EnvFileProcessor {
  ...
  _groupRolesPerOperationType(originalRoles, rolesToModify) {
    ...
    const originalRolesGroupedById = {};
    const originalRolesGroupedByName = {};
    const originalRolesDuplicatedNames = [];

    originalRoles.forEach((role) => {
      originalRolesGroupedById[role._id] = role;
      const name = role.name;
      if (originalRolesGroupedByName[name]) {
        originalRolesDuplicatedNames.push(name);
      }

      originalRolesGroupedByName[name] = role;
    });
