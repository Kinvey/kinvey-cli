...
const primarySchema = Joi.object().keys({
  ...
  configType: Joi.string().required().valid(ConfigType.APP, ConfigType.ENV, ConfigType.SERVICE)
});
...
const rapidSrvEnvsSchema = Joi.object().required()
  .pattern(
    rgxAnyExceptWhitespace,
    Joi.object()
...
const flexInternalSrvEnvsSchema = Joi.object().required()
  .pattern(
    rgxAnyExceptWhitespace,
    Joi.object().keys({
...
const flexExternalSrvEnvsSchema = Joi.object().required()
  .pattern(
    rgxAnyExceptWhitespace,
    Joi.object().keys({
      secret: Joi.string().required().min(2),
...
const serviceSchemaInApp = Joi.object().keys({
  schemaVersion: Joi.any().required().valid('1.0.0')
}).keys(serviceSchemaKeys)
  .keys({ configType: Joi.string().optional().valid(ConfigType.SERVICE) });

const rgxEnvName = /^\S.*\S$/; // at least 2 chars, doesn't start + end  with whitespace
const appSchema = primarySchema.keys({
  settings: Joi.object().optional().keys({
