...
const primarySchema = Joi.object().keys({
  ...
  configType: Joi.string().required().valid(ConfigType.ENV, ConfigType.SERVICE)
});
...
const srvEnvsSchema = Joi.object().required()
  .pattern(
    rgxAnyExceptWhitespace,
    Joi.object().keys({
      host: Joi.string().required(),
      version: Joi.string().optional().valid(['online', '2013']),
      connectionOptions: Joi.object().optional(),
      authentication: Joi.object().optional().keys({
        type: Joi.string().optional().valid(['ServiceAccount', 'ServiceAccountOAuth', 'MIC', 'WindowsServiceAccount', 'None', 'Basic']),
      }).unknown(),
  ...
  .min(1)
  .max(1)
  .when('type', { is: ConfigFiles.ServiceType.FLEX_INTERNAL, then: Joi.forbidden() })
  .when('type', { is: ConfigFiles.ServiceType.FLEX_EXTERNAL, then: Joi.forbidden() });
...
const serviceSchema = primarySchema.keys(({
  ...
  secret: Joi.string().min(2)
    .when('type', { is: ConfigFiles.ServiceType.FLEX_INTERNAL, then: Joi.required() })
    .when('type', { is: ConfigFiles.ServiceType.FLEX_EXTERNAL, then: Joi.required() }),
  host: Joi.string().min(1).when('type', { is: ConfigFiles.ServiceType.FLEX_EXTERNAL, then: Joi.required(), otherwise: Joi.forbidden() }),
  sourcePath: Joi.string().forbidden().when('type', { is: ConfigFiles.ServiceType.FLEX_INTERNAL, then: Joi.optional() }),
  environments: srvEnvsSchema
}));
