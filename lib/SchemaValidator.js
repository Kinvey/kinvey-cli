...
const { AllowedBasicCollPermissions, AllowedHooksCustomCollection, AllowedHooksUserCollection,
  AllowedRolesPermissions, AllowedServiceTypesToApply, AllowedCLIRuntimes,
...
AllowedHooksCustomCollection.forEach((hook) => { collHooksPlainObj[hook] = collHookSchema; });

const userCollHooksPlainObj = {};
AllowedHooksUserCollection.forEach((hook) => { userCollHooksPlainObj[hook] = collHookSchema; });
...
const envSchemaKeys = {
  settings: Joi.object().optional().keys({
    ...
    passwordReset: Joi.string().optional()
  }),
  ...
  collectionHooks: Joi.object({
    user: Joi.object().keys(userCollHooksPlainObj) // `user` collection has a different set of allowed hooks
  })
    .pattern(
      rgxCollName,
