  ...
  _createFlexService(source, options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this._createSvcEnvs(service.id, source.environments, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
    ...
    async.series([
      (next) => {
        this.servicesService.getById(serviceId, (err, originalService) => {
          ...
          updateData = ServiceFileProcessor._buildRapidDataServiceObjectWoEnv(source, {}, originalService);

          next();
        });
      },
      (next) => {
        // TODO: uncomment when backend works as expected
        return setImmediate(next);
        // this.servicesService.update(updateData, serviceId, next);
      },
      (next) => {
