...
// Probably replace with lodash function when npm is OK
Utils.pickBy = function pickBy(source, allowedKeys, predicate) {
  const result = {};
  const sourceKeys = Object.keys(source);
  sourceKeys.forEach((x) => {
    if (allowedKeys.includes(x) && predicate(x)) {
      result[x] = lodashCloneDeep(source[x]);
    }
  });
