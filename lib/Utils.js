...
const { ActiveItemTypes, DomainTypes, Errors, PromptMessages, ConfigFiles } = require('./Constants');
 ...
 * @param {string} filePath
 */
Utils.readJSONConfigFile = function readJSON(file, cb) {
  Utils._readJSONConfigFile(file, 'root', (err, config) => {
    logger.debug('Final configuration: ' + JSON.stringify(config, null, 4));
    cb(err, config);
  });
};

Utils._readJSONConfigFile = function readJSON(file, fieldPath, cb) {
  ...
  return async.waterfall([
    ...
    (json, next) => {
      ...
      Utils._processConfigFileValue(json, dirname, fieldPath, (err, processedValue) => {
        if (err) {
...
Utils._processConfigFileValue = function (obj, baseFolder, fieldPath, done) {
  if (Array.isArray(obj) || lodashIsObject(obj)) {
    async.forEachOfSeries(
      obj,
      (value, key, callback) => {
        // console.log(key + '    ' + JSON.stringify(value));
        Utils._processConfigFileValue(value, baseFolder, `${fieldPath}.${key}`, (err, processedValue) => {
    ...
    if (obj.indexOf(ConfigFiles.FILE_REFERENCE_PREFIX) === 0) {
      ...
      Utils._readJSONConfigFile(filePath, fieldPath, done);
    } else {
...
Utils.sortList = function sortList(list, property = 'name') {
  return lodashSortBy(list, (x) => {
    return x[property].toLowerCase();
  });
};
