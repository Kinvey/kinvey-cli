...
Utils.formatHost = function formatHost(host) {
  if (Utils.isNullOrUndefined(host) || Utils.isStringWhitespace(host)) {
    return 'https://manage.kinvey.com/';
  }
...
Utils.writeJSON = function writeJSON({ file, data, createParentDir, space }, done) {
  async.series([
    (next) => {
      if (!createParentDir) {
      ...
      Utils.mkdirp(file, next);
    },
    (next) => {
      const contents = JSON.stringify(data, null, space);
      Utils.writeFile(file, contents, next);
    }
  ], done);
};

Utils.writeConfigFile = function writeConfigFile(filePath, data, done) {
  Utils.writeJSON({ file: filePath, data, createParentDir: true, space: 4 }, (err) => {
    done(err, data);
  });
};
...
Utils.Endpoints = {
  ...
  push: (schemaVersion, envId) => `${Utils.Endpoints.envs(schemaVersion, envId)}/push`,
  ...
  commonCode: (schemaVersion, envId, name) => `${Utils.Endpoints.envs(schemaVersion, envId)}/business-logic/common${Utils.Endpoints.getIdPartFromId(name)}`,
  endpoints: (schemaVersion, envId, name) => `${Utils.Endpoints.envs(schemaVersion, envId)}/business-logic/endpoints${Utils.Endpoints.getIdPartFromId(name)}`,
  hooks: (schemaVersion, envId, collName, hookName) => `${Utils.Endpoints.envs(schemaVersion, envId)}/business-logic/collections/${collName}${Utils.Endpoints.getIdPartFromId(hookName)}`,
  collectionHooks: (schemaVersion, envId, collectionName) => {
    const namePart = collectionName ? `/${collectionName}` : '';
    return `${Utils.Endpoints.envs(schemaVersion, envId)}/business-logic/collections${namePart}`;
  },
  orgs: (schemaVersion, orgId) => `${Utils.Endpoints.version(schemaVersion)}/organizations${Utils.Endpoints.getIdPartFromId(orgId)}`,
  appsByOrg: (schemaVersion, orgId) => `${Utils.Endpoints.orgs(schemaVersion, orgId)}/apps`,
  ...
  identityProviders: schemaVersion => `${Utils.Endpoints.version(schemaVersion)}/identity-providers`,
  identityProvidersAuthenticate: (schemaVersion, providerId) => `${Utils.Endpoints.version(schemaVersion)}/identity-providers/${providerId}/authenticate`,
  user: schemaVersion => `${Utils.Endpoints.version(schemaVersion)}/user`
};
...
Utils.mkdirp = function mkdirp(pathToFile, done) {
  try {
    const filepath = path.resolve(pathToFile);
    filepath.split(path.sep).slice().reduce((prev, curr, i) => {
...
Utils.isEnvID = function isEnvID(value) {
  ...
  // kid_SklZwh7dN
  const validEnvIdLengths = [12, 13];
  const isId = validEnvIdLengths.includes(value.length) && value.startsWith('kid_');
  return isId;
};
...
Utils.getObjectByPicking = function getObjectByPicking(source, propsToPick) {
  return lodashPick(source, propsToPick);
};
...
Utils.pickBy = function pickBy(source, predicate) {
  return lodashPickBy(source, predicate);
};
...
Utils.getDefaultIdentityProvider = function getDefaultIdentityProvider() {
  return {
    id: 'Kinvey',
    name: 'Kinvey'
  };
 ...
 * @returns {String}
 */
Utils.generateSecretKey = function generateSecretKey() {
  let ret = '';
  const length = 58;
...
Utils.openBrowser = function openBrowser(url) {
  return open(url);
};
