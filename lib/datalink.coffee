...
class Datalink
  ...
  deploy: (dir, version, cb) =>
    ...
    # Prepare the request.
    req = request.post {
      url      : "/v2/apps/#{project.app}/data-links/#{project.datalink}/deploy"
  ...
  _execRecycle: (cb) ->
    request.post {
      url     : "/v2/apps/#{project.app}/data-links/#{project.datalink}/recycle"
  ...
  _execStatus: (job, cb) ->
    request.get {
      url     : "/v2/apps/#{project.app}/data-links/#{project.datalink}/deploy?job=#{job}"
      headers : { Authorization: "Kinvey #{user.token}" }
    }, cb
