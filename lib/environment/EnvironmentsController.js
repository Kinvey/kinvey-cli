...
const { getCommandNameFromOptions, isNullOrUndefined, mapFromSource } = require('../Utils');
...
class EnvironmentsController extends BaseController {
    ...
    async.series([
      ...
      (next) => {
        if (options.file) {
          ...
          this.cliManager.processConfigFile(OperationType.CREATE, info, next);
        } else {
          const newEnv = {
            name: options.name
          };
  ...
  update(options, done) {
    this.environmentsService.getActiveOrSpecified(options, (err, env) => {
      ...
      this.cliManager.processConfigFile(OperationType.UPDATE, info, (err, data) => {
        if (err) {
          return done(err);
        }
