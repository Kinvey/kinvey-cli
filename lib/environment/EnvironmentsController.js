...
class EnvironmentsController extends BaseController {
  ...
  clone(options, done) {
    ...
    async.series([
      ...
      (next) => {
        this.environmentsService.getByIdOrName(options.target, appId, (err, data) => {
          if (err) {
            return next(err);
          }
      ...
      (next) => {
        const data = {
          ...
          params:{
            ...
            serviceEnvironmentMapping:[]
          }
        };
        this.environmentsService.clone(data, next);
      }
    ], (err, results) => {
      if (err) {
        return done(err);
      ...
      const userFriendlyMsg = `Clone initiated. Job: ${result.jobId}`;
      const cmdResult = new CommandResult()
        .setRawData({ jobId: result.jobId })
