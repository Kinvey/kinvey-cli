...
const EnvFileExporter_1_0_0 = require('../exporter/EnvFileExporter_1_0_0');
const BaseController = require('../BaseController');
const CommandResult = require('../CommandResult');
const { ActiveItemType, AppOptionsName, CommonOptionsNames, ConfigType, EntityType, EnvOptionsName, Errors, Mapping, OperationType, BackendCollectionPermission } = require('../Constants');
    ...
    async.waterfall([
      (next) => {
        this.environmentsService.getActiveOrSpecified(options, (err, data) => {
          ...
          const envFileExporter = new EnvFileExporter_1_0_0(this.options);
          const exportOptions = { env: env };
          envFileExporter.export(exportOptions, next);
        });
      },
      (exportedConfig, next) => {
        //console.log(JSON.stringify(exportedConfig, null, 4));

        const filename = options.file;
        fs.writeFileSync(filename, JSON.stringify(exportedConfig, null, 4));

        cmdResult.setBasicMsg(OperationType.EXPORT, EntityType.ENV, env.id);
        cmdResult.setRawData({ id: env.id, config: exportedConfig });
