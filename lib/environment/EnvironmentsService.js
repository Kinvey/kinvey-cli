...
const { Endpoints, getItemError, getTransformedError, getUsingEntityMsg, isNullOrUndefined } = require('./../Utils');
...
class EnvironmentsService extends BaseService {
  ...
  getByIdOrName(identifier, appId, done) {
    ...
    super.getEntityByIdOrName(identifier, endpointAll, endpointId, (err, entity) => {
      if (err) {
        return done(getTransformedError(err, EntityType.ENV, identifier));
      }
  ...
  getById(id, done) {
    ...
    this.cliManager.sendRequest({ endpoint }, (err, entity) => {
      if (err) {
        return done(getTransformedError(err, EntityType.ENV, id));
      }
  ...
  getAppId(appIdentifier, done) {
    ...
    super.getEntityByIdOrName(appIdentifier, endpointAll, endpointId, (err, data) => {
      if (err) {
        return done(getTransformedError(err, EntityType.APP, appIdentifier));
      }
