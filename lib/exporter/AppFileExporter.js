...
class AppFileExporter {
  ...
  exportApp(options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this._exportServices({ dir, appId: rawData.id }, (err, exportedServices) => {
          if (err) {
            return next(err);
          }
        ...
        async.eachSeries(
          ...
          (env, cb) => {
            ...
            this.envExporter.export({ env, filename: filePath }, (err) => {
              ...
              exportedEnvs[env.name] = `${ConfigFiles.FILE_REFERENCE_PREFIX}./${path.basename(options.dir)}/${fileName}`;
              cb();
            });
          }, next);
      }
  ...
  _exportServices(options, done) {
    ...
    async.waterfall([
      (next) => {
        this.servicesService.getAllByDomainType(DomainTypes.APP, { id: options.appId }, next);
      },
      (services, next) => {
        async.eachSeries(
          services,
