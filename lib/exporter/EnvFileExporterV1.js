...
const { isEmpty, isNullOrUndefined, mkdirp } = require('../Utils');
...
class EnvFileExporterV1 {
  ...
  _transformCollectionHook(apiCollectionHook, collectionName, services, dirname, done) {
    ...
    if (apiCollectionHook.host) {
      ...
      this._transformCode(apiCollectionHook.code, hookObject, dirname, filename, (err, result) => done(err, hookObject));
    }
  }
  ...
  _transformCustomEndpoint(apiCustomEndpoint, services, dirname, done) {
    ...
    if (apiCustomEndpoint.host) {
      ...
      this._transformCode(apiCustomEndpoint.code, endpointObject, dirname, filename, err => done(err, endpointObject));
    }
  }
  ...
  _transformCommonCodeScript(apiCommonCode, dirname, done) {
    ...
    this._transformCode(apiCommonCode.code, endpointObject, dirname, filename, err => done(err, endpointObject));
  }

  _transformCode(sourceCodeString, targetObject, dirname, filename, done) { // eslint-disable-line
    if (filename) {
