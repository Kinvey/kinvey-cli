        ...
        async.forEachSeries(
          ...
          (collection, iter) => {
            if (EnvFileExporterV1._mustExportCollection(collection.name)) {
              this._transformCollection(collection, apiRoles, (err, configCollection) => {
        ...
        async.forEachSeries(
          ...
          (collectionHook, nextCollection) => {
            ...
            async.forEachSeries(
              hooks,
              (hook, nextHook) => {
                this._transformCollectionHook(hook, services, (err, transformedHook) => {
    ...
    async.series([
      (callback) => {
        if (apiCollection.dataLink) {
          ...
          this.servicesService.getById(apiCollection.dataLink.id, (err, service) => {
