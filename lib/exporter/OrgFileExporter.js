...
const { AppOptionsName, ConfigFiles, ConfigType, DomainTypes, LogLevel, OrgOptionsName } = require('../Constants');
const { isEmpty, mkdirp, writeConfigFile } = require('../Utils');
...
class OrgFileExporter {
  ...
  exportOrg(options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this._exportApps({ dir, orgId: fetchedOrg.id }, (err, exportedApps) => {
          if (err) {
            return next(err);
          }
      ...
      (next) => {
        ...
        this._exportServices({ dir, org: fetchedOrg }, (err, exportedServices) => {
          if (err) {
            return next(err);
          }
        ...
        async.eachSeries(
          ...
          (currApp, cb) => {
            ...
            this.appExporter.exportApp({ file: filePath, [AppOptionsName.APP]: currApp.id }, (err) => {
              ...
              result[appName] = `${ConfigFiles.FILE_REFERENCE_PREFIX}./${path.basename(options.dir)}/${appName}/${fileName}`;
              cb();
            });
          },
          next
   ...
   * @param {String} options.org Organization from the backend.
   * @param done
   * @private
   */
  _exportServices(options, done) {
    ...
    async.series([
      (next) => {
        this.servicesService.getAllOwnedByOrg(options.org.id, false, (err, data) => {
          if (err) {
            return next(err);
          }
        ...
        async.eachSeries(
          ...
          (currService, cb) => {
            ...
            this.serviceExporter.exportService({ file: filePath, service: currService }, (err) => {
              ...
              result[serviceName] = `${ConfigFiles.FILE_REFERENCE_PREFIX}./${path.basename(options.dir)}/${fileName}`;
              cb();
            });
          },
          next
