...
class ServiceFileExporter {
  ...
  exportService(done) {
    ...
    this.servicesService.getById(this.serviceId, (err, data) => {
      ...
      try {
        fs.writeFileSync(this.filepath, JSON.stringify(result, null, 4));
      } catch (ex) {
        return done(ex);
      }
    ...
    if (Array.isArray(service.backingServers) && !isEmpty(service.backingServers[0])) {
      ...
      serviceObjectNames.forEach((serviceObjName) => {
        ...
        if (envMapping[serviceObjName].sourceObject) {
          // sometimes sourceObject contains more props than needed, so let's just take some
          const sourceObjProps = ['objectName', 'schemaName', 'objectType', 'primaryKey', 'name'];
