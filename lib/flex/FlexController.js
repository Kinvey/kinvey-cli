...
class FlexController extends BaseController {
  ...
  _saveJobId(id, done) {
    ...
    this.projectSetup.save((err) => {
      if (err) {
        this.cliManager.log(LogLevel.DEBUG, 'Failed to save job ID to project settings.');
      } else {
  ...
  list(options, done) {
    async.series([
      ...
      const result = results.pop();
      this.cliManager.processCommandResult(new CommandResult().setError(err).setRawData(result), done);
    });
  }
