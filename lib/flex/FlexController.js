...
const { getCommandNameFromOptions, isEmpty, isNullOrUndefined } = require('../Utils');
...
class FlexController extends BaseController {
  ...
  _saveJobId(key, id, done) {
    ...
    this.projectSetup.save((err) => {
      if (err) {
        this.cliManager.log(LogLevel.WARN, 'Failed to save job ID to project settings.');
        this.cliManager.log(LogLevel.ERROR, err);
      } else {
  ...
  _getValidationErr(keys) {
    keys = Array.isArray(keys) ? keys : [keys];
    return new KinveyError(Errors.ProjectNotConfigured.NAME, `${Errors.ProjectNotConfigured.MESSAGE} ${keys.join(', ')}.`);
  }
  ...
  preProcessOptions(options) {
    ...
    if (serviceIdIsRequired) {
      ...
      if (isNullOrUndefined(this._metadata[FlexOptionsNames.SERVICE_ID])) {
        throw this._getValidationErr(FlexOptionsNames.SERVICE_ID);
      }
    }
    ...
    if (jobIdIsRequired) {
      ...
      if (isNullOrUndefined(this._metadata.jobId)) {
        throw this._getValidationErr(FlexOptionsNames.JOB_ID);
      }
    }
    ...
    if (domainIdIsRequired) {
      ...
      if (isNullOrUndefined(this._metadata.domain) || isNullOrUndefined(this._metadata.domainEntityId)) {
        throw this._getValidationErr([FlexOptionsNames.DOMAIN_TYPE, FlexOptionsNames.DOMAIN_ID]);
      }
    }
