...
const { formatList, generateSecretKey, getCommandNameFromOptions, getObjectFromDelimiterSeparatedKeyValuePairs, isEmpty, isValidNonZeroInteger, isValidTimestamp, isNullOrUndefined } = require('../Utils');
...
class FlexController extends BaseController {
  ...
  _transformEnvVars(options) {
    ...
    try {
      ...
      throw new Error('Environment variables must be specified as comma-separated list where key=value or in valid JSON format.');
    }
  }
  ...
  deploy(options, done) {
    ...
    async.series([
      (next) => {
        ...
        this.servicesService.getServiceById(serviceId, (err, data) => {
          if (err) {
            return next(err);
          }
      ...
      (next) => {
        ...
        const serviceId = this._metadata.serviceId;
        this.servicesService.deployFlexProject({ dir, serviceId, schema, envVars: finalEnvVars }, (err, deployJobId) => {
          if (err) {
            return next(err);
          }
  ...
  update(options, done) {
    async.waterfall([
      (next) => {
        this.servicesService.getById(this._metadata.serviceId, next);
      },
      (existingEntity, next) => {
        if (!isNullOrUndefined(options[FlexOptionsNames.ENV_VARS_REPLACE])) {
        ...
        this.servicesService.update(existingEntity, this._metadata.serviceId, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
