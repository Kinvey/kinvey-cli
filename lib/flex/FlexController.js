...
const { AppOptionsName, Command, CommonOptionsNames, DeploymentStatus, Errors, EntityType, DomainTypes, InfoMessages, LogErrorMessages, LogLevel,
  OperationType, OrgOptionsName, PromptMessages, PromptTypes, ServiceStatus, ServiceOptionsNames, FlexOptionsNames, FlexProjectMaxSize, JobStatus } = require('../Constants');
const CommandResult = require('../CommandResult');
const KinveyError = require('../KinveyError');
const { formatList, generateSecretKey, getCommandNameFromOptions, getCustomNotFoundError, isEmpty, isValidNonZeroInteger, isValidTimestamp, isNullOrUndefined } = require('../Utils');
...
class FlexController extends BaseController {
  ...
  _findSvcEnvId(options, done) {
    ...
    this.servicesService.getSpecifiedOrDefaultSvcEnv(this._metadata[FlexOptionsNames.SERVICE_ID], options[ServiceOptionsNames.SVC_ENV], (err, env) => {
      if (err) {
        return done(err);
      }
  ...
  _getInitInput(done) {
    ...
    async.waterfall([
      ...
      (entity, next) => {
        ...
        this.servicesService.getAllInternalFlexServicesByDomain(entity, next);
      },
      (services, next) => {
  ...
  deploy(options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this.servicesService.deployFlexProject(dir, serviceId, svcEnvId, schema, (err, deployJobId) => {
          if (err) {
            return next(err);
          }
  ...
  getTableDataFromStatusData(data, { serviceId, serviceName }) {
    ...
    if (hasDeployment) {
      result.runtime = (data.runtime || 'Unknown').toUpperCase();
    }
    ...
    if (hasDeployment) {
      ...
      result.deploymentRuntime = (data.deployment.runtime || 'Unknown').toUpperCase();
    }
  ...
  list(options, done) {
    ...
    async.waterfall([
      (next) => {
        if (isDomainApp) {
          this.applicationsService.getByIdOrName(domainEntity.id, next);
        } else {
          this.organizationsService.getByIdOrName(domainEntity.id, next);
        }
      },
      (fetchedEntity, next) => {
        this.servicesService.getAllInternalFlexServicesByDomain(fetchedEntity, next);
      }
    ], (err, services) => {
      if (err) {
        return done(err);
  ...
  recycle(options, done) {
    ...
    async.series([
      ...
      (next) => {
        this.servicesService.execRecycle(this._metadata.schemaVersion, this._metadata.serviceId, this._metadata[ServiceOptionsNames.SVC_ENV], (err, recycleJobId) => {
          if (err) {
            return next(err);
          }
