...
class FlexController extends BaseController {
  ...
  _validateOrgIsAvailable(options, projectSetup, checkActive) {
  ...
  _validateCommandOptions(cmd, options, checkActive, projectSetup = {}) {
    ...
    if (orgIdIsRequired) {
      return this._validateOrgIsAvailable(options, projectSetup, checkActive);
    }
  }
  ...
  preProcessOptions(options) {
    ...
    const errValidation = this._validateCommandOptions(cmd, options, true, loadedSetup);
    if (errValidation) {
      throw errValidation;
    }
  ...
  deploy(options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this.servicesService.deployFlexProject(deployOpts, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
      ...
      const userFriendlyMsg = `Deploy initiated. You can use 'kinvey ${Command.FLEX_STATUS}' to track the deploy progress.`;
      const cmdResult = new CommandResult()
  ...
  create(options, done) {
    ...
    async.waterfall([
      ...
      (service, next) => {
        ...
        this.servicesService.createSvcEnv(serviceId, data, next);
      }
    ], (err, result) => {
      if (err) {
        return done(err);
  ...
  recycle(options, done) {
    ...
    async.series([
      ...
      (next) => {
        this.servicesService.execRecycle(this._metadata.schemaVersion, this._metadata.serviceId, this._metadata.svcEnvId, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
      ...
      const userFriendlyMsg = `Recycle initiated. You can use 'kinvey ${Command.FLEX_STATUS}' to track the recycle progress.`;
      const cmdResult = new CommandResult()
        .setCustomMsg(userFriendlyMsg);
      done(null, cmdResult);
    });
