...
const { AppOptionsName, APIRuntimeToCLIRuntime, Command, CommonOptionsNames, CLIRuntime, CLIRuntimeToAPIRuntime, DeploymentStatus,
  Errors, EntityType, DomainTypes, InfoMessages, LogErrorMessages, LogLevel, OperationType, OrgOptionsName,
  PromptMessages, PromptTypes, ServiceStatus, FlexOptionsNames, JobStatus } = require('../Constants');
...
class FlexController extends BaseController {
  ...
  deploy(options, done) {
    ...
    async.series([
      ...
      (next) => {
        const schemaVersion = this._metadata.schemaVersion;
        const serviceId = this._metadata.serviceId;
        const runtime = CLIRuntimeToAPIRuntime[options[FlexOptionsNames.RUNTIME]];
        this.servicesService.deployFlexProject({ dir, serviceId, schemaVersion, runtime, envVars: finalEnvVars }, (err, deployJobId) => {
          if (err) {
            return next(err);
          }
  ...
  show(options, done) {
    this.servicesService.getById(this._metadata.serviceId, (err, data) => {
      ...
      if (data.backingServers && data.backingServers[0]) {
        ...
        if (data.backingServers[0].runtime) {
          tableData.runtime = APIRuntimeToCLIRuntime[data.backingServers[0].runtime];
        }
