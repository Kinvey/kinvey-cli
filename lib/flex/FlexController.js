...
const { AppOptionsName, Command, CommonOptionsNames, Errors, EntityType, DomainTypes, LogLevel, OperationType, OrgOptionsName,
  ServiceOptionsNames, FlexOptionsNames, FlexProjectMaxSize, JobStatus } = require('../Constants');
  ...
  deploy(options, done) {
    ...
    async.series([
      ...
      (next) => {
        ...
        this.flexService.deployProject(dir, serviceId, svcEnvId, version, schema, uploadTimeout, FlexProjectMaxSize, artifacts, finalEnvVars, (err, deployJobId) => {
          if (err) {
            return next(err);
          }
