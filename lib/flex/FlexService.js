...
const { Endpoints, formatList, findAndSortInternalServices, isArtifact, isEmpty, isStringWhitespace, isNullOrUndefined, isValidNonZeroInteger, isValidTimestamp, readJSON } = require('../Utils');
...
class FlexService {
  ...
  _printServiceInfo(serviceId, serviceName, data) {
    ...
    const statusLastPart = isNullOrUndefined(serviceName) ? '' : ` (${chalk.gray(serviceName)})`;
    console.log(`Status of FSR service ${chalk.cyan(serviceId)}${statusLastPart}`);
    ...
    if (hasDeployInfo) {
      process.stdout.write('\n');
      const formattedDate = moment(data.requestedAt).format('dddd, MMMM Do YYYY, h:mm:ss A');
      console.log(`  Requested on:   ${formattedDate}`);
  ...
  printServiceLogs({ serviceId, serviceName }, logs) {
    ...
    const serviceNamePart = isNullOrUndefined(serviceName) ? '' : ` (${chalk.gray(serviceName)})`;
    if (skippedLogEntries.length > 0) {
      this.cliManager.log(LogLevel.DEBUG, `${skippedLogEntries.length} skipped log entries for FSR service ${serviceId}${serviceNamePart}: ${JSON.stringify(skippedLogEntries)}`);
    }

    const logsCount = chalk.cyan(logs.length - skippedLogEntries.length);
    console.log(`Query returned ${logsCount} logs for FSR service ${chalk.cyan(serviceId)}${serviceNamePart}`);
  }
