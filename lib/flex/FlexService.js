...
const { DomainTypes, Errors, EntityType, FlexOptionsNames, HTTPMethod, InfoMessages, LogErrorMessages, LogLevel,
  PromptMessages, PromptTypes, ServiceStatus, DeploymentStatus } = require('../Constants');
...
class FlexService extends BaseService {
   ...
   * @private
   */
  getServices(entity, done) {
    let queryString = entity.security ? `organizationId=${entity.id}` : `appId=${entity.id}`;
    if (!isNullOrUndefined(entity.organizationId)) {
      queryString += `&&organizationId=${entity.organizationId}`;
    }

    const endpoint = `${Endpoints.services(this.cliManager.config.defaultSchemaVersion)}?${queryString}`;
    this.cliManager.sendRequest({ endpoint }, (err, data) => {
      if (err) {
        return done(err);
      }
  ...
  getInitInput(done) {
    ...
    async.waterfall([
      ...
      (serviceId, next) => {
        this.getSvcEnvChoice(serviceId, (err, env) => {
          if (err) {
            return next(err);
          }
  ...
  getSvcEnvChoice(serviceId, done) {
  ...
  deployProject(dir, serviceId, svcEnvId, pkgVersion, schemaVersion, uploadTimeout, maxUploadSize, artifacts, envVars, done) {
