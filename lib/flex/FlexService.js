...
class FlexService {
  ...
  _getAllDomainEntities(done) {
    ...
    async.series([
      (next) => {
        this._getDomainEntities(false, (err, data) => {
          if (err) {
      ...
      (next) => {
        this._getDomainEntities(true, (err, data) => {
          if (err) {
  ...
  _printServiceInfo(serviceId, serviceName, data) {
    ...
    const statusLastPart = isStringWhitespace(serviceName) ? '' : ` (${chalk.gray(serviceName)})`;
  ...
  printServiceLogs({ serviceId, serviceName = '' }, logs) {
    ...
    const lastPart = isStringWhitespace(serviceName) ? '' : ` (${chalk.gray(serviceName)})`;
    console.log(`Query returned ${logsCount} logs for FSR service ${chalk.cyan(serviceId)}${lastPart}`);
  }
