...
class InitController extends BaseController {
  ...
  _createProfileOnce(input, done) {
    async.series([
      ...
      (next) => {
        const formattedHost = formatHost(input[AuthOptionsNames.HOST]);
        this.cliManager.createProfile(input[AuthOptionsNames.PROFILE], input[AuthOptionsNames.EMAIL], input[AuthOptionsNames.PASSWORD], input.MFAToken, formattedHost, next);
      }
    ], (err) => {
      done(err, input);
    });
  ...
  _tryToCreateProfileRetryable(done) {
    ...
    async.doUntil(
      ...
      (err) => {
        ...
        done(null, input[AuthOptionsNames.PROFILE]);
      }
    );
  }
   ...
   */
  handleInit(done) {
    this._tryToCreateProfileRetryable((err, profileName) => {
      if (err) {
        return this.cliManager.processCommandResult(err);
      }

      const cmdResult = new CommandResult()
        .setRawData({ id: profileName })
        .setBasicMsg(OperationType.CREATE, EntityType.PROFILE);
      this.cliManager.processCommandResult(null, cmdResult);
    });
  }
}
