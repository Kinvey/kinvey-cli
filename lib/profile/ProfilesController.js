...
const { AuthOptionsNames, EntityType, Errors, LogLevel, Mapping, OperationType, PromptMessages, PromptTypes } = require('../Constants');
const CommandResult = require('../CommandResult');
const KinveyError = require('../KinveyError');
const { getDefaultIdentityProvider, getItemError, isEmpty, isMFATokenError, isNullOrUndefined, mapFromSource, sortList, validateMFAToken, validateString } = require('../Utils');
...
class ProfilesController extends BaseController {
  ...
  create(options, done) {
    ...
    this.cliManager.createProfile(
      profileName,
      {
        identityProvider: getDefaultIdentityProvider(),
        email: options[AuthOptionsNames.EMAIL],
        password: options[AuthOptionsNames.PASSWORD],
        MFAToken: options[AuthOptionsNames.TWO_FACTOR_AUTH_TOKEN],
        host: options[AuthOptionsNames.HOST]
      },
      (err) => {
  ...
  show(options, done) {
    ...
    const modifiedProfile = mapFromSource(Mapping[EntityType.PROFILE].DETAILS, profile);
  ...
  login(options, done) {
    ...
    async.series([
      (next) => {
        if (profile.identityProvider.name !== 'Kinvey') {
        ...
        this.cliManager.prompter.prompt(q, (err, data) => {
          ...
          input[AuthOptionsNames.PASSWORD] = data[AuthOptionsNames.PASSWORD];
          next();
        });
      },
      (next) => {
        this.cliManager.reAuthenticateProfile(profileName, input[AuthOptionsNames.PASSWORD], null, (err) => {
          if (err) {
            if (isMFATokenError(err)) {
              askForMFA = true;
              return setImmediate(next);
      ...
      (next) => {
        ...
        this.cliManager.reAuthenticateProfile(profileName, input[AuthOptionsNames.PASSWORD], input.mfa, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
