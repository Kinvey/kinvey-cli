...
const { getItemError, isEmpty, isMFATokenError, isNullOrUndefined, sortList, validateMFAToken, validateString } = require('../Utils');
...
class ProfilesController extends BaseController {
  ...
  login(options, done) {
    ...
    async.series([
      ...
      (next) => {
        this.cliManager.createProfile(profileName, profile.email, input[AuthOptionsNames.PASSWORD], null, profile.host, (err) => {
          if (err) {
            if (isMFATokenError(err)) {
              askForMFA = true;
      ...
      (next) => {
        ...
        this.cliManager.createProfile(profileName, profile.email, input[AuthOptionsNames.PASSWORD], input.mfa, profile.host, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
