...
const { isEmpty, isMFATokenError, isNullOrUndefined, validateString } = require('../Utils');
...
class ProfilesController extends BaseController {
  ...
  login(options, done) {
    console.log(options);
    // TODO: cli-53 adjust
    const profileToUse = isNullOrUndefined(options.name) ? this.cliManager.getCurrentProfileName() : options.name;
    const profile = this.cliManager.findProfile(profileToUse);
    if (!profile) {
      // Maybe use ItemNotSpecified
      return done(new Error('Do smth!'));
    }
    ...
    async.waterfall([
      ...
      (password, next) => {
        this.cliManager.createProfile(profileToUse, profile.email, password, null, profile.host, next);
      }
    ], (err) => {
      if (err) {
        return done(err);
