...
class Project {
  ...
  restore(cb) {
    ...
    return util.readJSON(this.projectPath, (err, data) => {
      if (err != null) return cb(new KinveyError(Errors.ProjectRestoreError));
  ...
  _selectApp(cb) {
    return async.waterfall([
      ...
      (apps, next) => {
        if (apps.length === 0) return next(new KinveyError(Errors.NoAppsFound));
  ...
  _selectOrg(cb) {
    return async.waterfall([
      ...
      (orgs, next) => {
        if (orgs.length === 0) return next(new KinveyError(Errors.NoOrgsFound));
  ...
  _selectService(cb) {
    return async.waterfall([
      ...
      (services, next) => {
        if (services.length === 0) return next(new KinveyError(Errors.NoFlexServicesFound));
