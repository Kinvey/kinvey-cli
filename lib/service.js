  ...
  deploy(dir, version, cb) {
    ...
    archive.on('data', () => {
      ...
      if (size > config.maxUploadSize) {
        ...
        return req.emit('error', new KinveyError('ProjectMaxFileSizeExceeded'));
      }
    });
  ...
  serviceStatus(cb) {
    return this._execServiceStatus((err, response) => {
      ...
      if (status === STATUS_CONSTANTS.ONLINE) {
        status = chalk.greenBright(STATUS_CONSTANTS.ONLINE);
      } else if (status === STATUS_CONSTANTS.UPDATING) {
        status = chalk.yellowBright(STATUS_CONSTANTS.UPDATING);
      } else if (status === STATUS_CONSTANTS.NEW) {
        status = chalk.cyanBright(STATUS_CONSTANTS.NEW);
      } else if (status === STATUS_CONSTANTS.ERROR) {
        status = chalk.redBright(STATUS_CONSTANTS.ERROR);
      }
      ...
      if (response.body.version != null) {
        ...
        logger.warn('\'kinvey status\' now supports displaying the service version in addition to service health. Please initiate a one-time recycle to enable this feature. See the README or contact Kinvey support for more information.');
      }

      cb(null, { status: response.body.status, version: response.body.version });
    });
  }
