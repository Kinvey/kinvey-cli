  ...
  serviceStatus(cb) {
    return this._execServiceStatus((err, response) => {
      ...
      if (status === ServiceStatus.ONLINE) {
        status = chalk.green(ServiceStatus.ONLINE);
      }
      if (status === ServiceStatus.UPDATING) {
        status = chalk.yellow(ServiceStatus.UPDATING);
      }
      if (status === ServiceStatus.NEW) {
        status = chalk.cyan(ServiceStatus.NEW);
      }
      if (status === ServiceStatus.ERROR) {
        status = chalk.red(ServiceStatus.ERROR);
      }
