...
class ServicesService extends BaseService {
  ...
  _validateFlexProjectPackageJson(json, serviceId, schemaVersion, done) {
    ...
    this.getServiceStatus({ serviceId, schemaVersion }, (err, data) => {
      if (err) {
        return done(err);
      }
      ...
      try {
        ...
        // TODO: consider removing these logs before publishing
        this.cliManager.log(LogLevel.DEBUG, JSON.stringify(ex, null, 2));
  ...
  deployFlexProject({ dir, serviceId, schemaVersion, envVars }, done) {
    async.waterfall([
      (next) => {
      ...
      (packageJson, next) => {
        this._validateFlexProjectPackageJson(packageJson, serviceId, schemaVersion, (err) => {
          if (err) {
            return next(err);
          }
