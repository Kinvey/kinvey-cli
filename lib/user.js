...
class User {
  ...
  login(email, password, token, cb) {
    ...
    if (useEnvVariables) {
      ...
      return this._loginOnce(envEmail, envPassword, cb);
    }
  ...
  _retryLogin(email, password, token, cb) {
    async.doUntil(
      (next) => {
        this._loginOnce(email, password, (err) => {
          if (err) {
            if (err.name === 'InvalidTwoFactorAuth') {
              this.mfaLogin(email, password, token, next);
            } else {
              email = password = null;
              next(err);
            }
  ...
  _loginOnce(email, password, cb) {
    return async.waterfall([
      ...
      if (err) {
        if (err.name === 'InvalidCredentials' || err.name === 'ValidationError') {
          return cb();
        }
