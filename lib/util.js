...
function makeRequest(options, cb) {
  ...
  return request.Request(options, (err, response) => {
    ...
    if (err != null) {
      if (err.message.indexOf('ENOTFOUND') !== -1) return cb(new KinveyError(Errors.InvalidConfigUrl));
      for (let i = 0; i < connErrors.length; i++) {
        const msg = connErrors[i];
        if (err.message.indexOf(msg) !== -1) return cb(new KinveyError(Errors.ConnectionError));
      }
      return cb(err);
    }
    ...
    if (response.body) {
      ...
      return cb(new KinveyError(response.body.code, errMsg));
    }

    return cb(new KinveyError(Errors.RequestError.NAME, response.statusCode));
  });
}
...
// FIXME: Refactor so that cb receives err and data
function handleCommandFailure(err, cb) {
  if (err != null) {
    logger.error('%s', err);
