...
class SiteEnvsService extends BaseService {
  ...
  deploy({ siteId, siteEnvId, targetPath }, done) {
    SiteEnvsService.buildFormData(targetPath, (err, formData) => {
      ...
      const req = this.cliManager.sendRequest({
        ...
        formData,
        timeout: this.cliManager.config.flexProjectUploadTimeout // TODO: cli-161 Choose an appropriate timeout
      }, (err, data) => {
        if (err) {
          return done(err);
        }
  ...
  status(siteId, siteEnvId, done) {
    const endpoint = Endpoints.siteStatus(this.cliManager.config.defaultSchemaVersion, siteId, siteEnvId);
    this.cliManager.sendRequest({ endpoint }, done);
  }
}
