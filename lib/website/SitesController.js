...
const { CommonOptionsNames, EntityType, Errors, OperationType, OrgOptionsName, SitesOptionsNames } = require('../Constants');
const KinveyError = require('../KinveyError');
const { isEmpty, isNullOrUndefined, sortList } = require('../Utils');
...
class SitesController extends BaseController {
    ...
    this.sitesService.getAll((err, data) => {
      ...
      const rawData = data.map((x) => {
        return {
          id: x.id,
          name: x.name,
          organizationId: x.organizationId || 'Not set'
        };
      });
  ...
  show(options, done) {
    ...
    this._getSiteWithDefaultSiteEnv({ siteIdentifier }, (err, data) => {
      ...
      const rawResult = {
        ...
        organizationId: data.organizationId || 'Not set'
      };
  ...
  deploy(options, done) {
    ...
    async.series([
      ...
      if (!isNullOrUndefined(result.cdnUrl)) {
        finalResult.cdnUrl = result.cdnUrl;
      }

      if (!isNullOrUndefined(result.internalUrl)) {
        finalResult.internalUrl = result.internalUrl;
      }
