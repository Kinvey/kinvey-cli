...
class SitesController extends BaseController {
  ...
  _getAccessField(options, done) {
    if (!isNullOrUndefined(options[AppOptionsName.APP])) {
      this.applicationsService.getByIdOrName(options[AppOptionsName.APP], (err, app) => {
        if (err) {
          return done(err);
        }
    ...
    } else if (!isNullOrUndefined(options[OrgOptionsName.ORG])) {
      ...
      done(new KinveyError(Errors.AppOrOrgRequired));
    }
  }
