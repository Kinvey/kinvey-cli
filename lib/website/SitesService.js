...
class SitesService extends BaseService {
  getByIdOrName(identifier, done) {
    const endpointAll = Endpoints.sites(this.cliManager.config.defaultSchemaVersion);
    const endpointId = Endpoints.sites(this.cliManager.config.defaultSchemaVersion, identifier);
    super.getEntityByIdOrName(identifier, endpointAll, endpointId, (err, entity) => {
      if (err) {
        return done(getTransformedError(err, EntityType.SITE, identifier));
      }
      done(null, entity);
    });
  }
